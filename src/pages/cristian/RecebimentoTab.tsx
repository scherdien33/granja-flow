// RecebimentoTab.tsx — Confirmação de recebimento de pedidos comprados
...
import { updatePedido, formatDate, type Pedido, type Condicao } from "@/lib/pedidos";
...
export function RecebimentoTab({ pedidos, onRefresh }: Props) {
            ...
            {aguardando.map((p) => (
              <div key={p.id} className="rounded-md border bg-card p-4 shadow-sm">
                ...
                {expandedId === p.id ? (
                  ...
                  <Button variant="outline" size="sm" className="mt-3" onClick={() => openForm(p.id)}>
